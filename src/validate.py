...
class Tester:
    ...
    def test_kg(self, kg_name, is_val=True, is_filtered=False):
        ...
        for kg_batch, ground_truth_batch in zip(kg_batch_generator, ground_truth_generator):
            ...
            r_embedding = self.model.predict_r_embedding(r_batch) #[b, e]

            model_predictions = self.model.predict(h_embedding, r_embedding) # [b, 1, e]
            candidate_emb = self.model.predict_candidate(kg.computed_entity_embedding_kg) # [b, a, e]

            ranking_indices, ranking_scores = ranking_all_batch(model_predictions, candidate_emb, define_score=self.model.predict_score_fuc())
            ...
            if is_filtered and not is_val:
                for i in range(h_batch.shape[0]):
                    ...
                    if (h, r) in hr2t_train:
                        ...
                        # _, idx = p_1.sort(stable=True) # torch 1.8
                        _, idx = p_1.sort()
        ...
        print('time:', kg_name, (time.time() - time0), num_samples,  (time.time() - time0)/num_samples)
    ...
    def pre_compute_all_embeddings(self, kg_name):
        with torch.no_grad():
            ...
            for data in dataloader:
                ...
                node_embeddings, _ = self.model.forward_GNN_embedding(graph_batch_list, kg_index)
