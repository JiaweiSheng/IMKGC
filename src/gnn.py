...
def softmax(src, index, num_nodes=None):
    """
    # softmax(attention, edge_index_i, num_nodes)
...
class MGA(MessagePassing):
    ...
    def message(self, x_j, x_i, edge_index_i, edge_kg_index, edge_beta_r, edge_relation_embedding, num_nodes):
        ...
        x_j_transfer = F.gelu(self.compute_transfer(torch.cat([x_j, edge_relation_embedding], dim=1), self.w_transfer_list, self.d_input, edge_kg_index_j)) # [num_edges, d_input]
    ...
    def compute_transfer(self, input_x, linear_transfer, output_dim, input_edge_kg_index):
        ...
        for i in range(len(linear_transfer)): # kg number
            idx_i = torch.nonzero(input_edge_kg_index == i).view(-1)
            input_x_i = input_x[idx_i] 
...
class GNN(nn.Module):
    ...
    def get_real_index(self, y, s):
        # y:[entity number]，s:[node number in the subgraph]
