...
def save_int_tensor_to_txt(tensor, file_path):
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    tensor_cpu = tensor.cpu().detach()
    if tensor_cpu.dim() > 1:
        tensor_cpu = tensor_cpu.squeeze()  
    np_array = tensor_cpu.numpy()
    np.savetxt(file_path, np_array, fmt='%d')  



def load_model(ckpt_path, model=None, device='cpu'):
    ...
    if model is None:
        raise ValueError("未提供模型实例。请根据返回的args重建模型后加载权重")
...
def ranking_all_batch(predicted_t, embedding_matrix, k = None, define_score = None, d_r = None):
    ...
    if define_score is not None:
        # d_r: [b, 1, 1]
        distance = define_score([predicted_t, embedding_matrix], d_r)
    else:
        distance = torch.norm(predicted_t - embedding_matrix, dim=2)  # [b,a]

    num_total_entities = embedding_matrix.size(1)
    if k== None:
...
def get_all_edges(kg_dir, kg_objects_dict, all_entity_global_index):
    ...
    for language in kg_objects_dict:
        edge_index, edge_relation = get_kg_edges_for_each(kg_dir, language) # [2, num_edges], [num_edges]
...
def nodes_to_k_graph(k_subgraph_list, node_index, device, shuffle=False):
    '''
    k_subgraph_list: entity number, KG number
    '''
    batch_size = node_index.shape[0] 
    graph_batches = []
