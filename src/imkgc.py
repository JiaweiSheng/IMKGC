...
class IMKGC(nn.Module):
    def __init__(self, args):
        ...
        self.rvq = ResidualVectorQuantizationVanilla(num_codebooks=self.args.reason_step, codebook_size=int(
            self.num_relations*self.args.codebook_ratio), codebook_dim=self.relation_dim, commitment_cost=self.args.commit_loss)
    ...
    def forward_GNN_embedding(self, graph_input_list, kg_index):
        ...
        for i, graph_input in enumerate(graph_input_list):
            ...
            x_gnn_output = self.encoder_KG(x_features, edge_index, edge_kg_index, edge_beta_r,
    ...
    def forward_kg(self, h_graph, sample, t_graph, t_neg_graph, kg_index):
        ...
        for i in range(self.num_kgs):
            ...
            kg_loss = self.criterion(pos_loss.expand(bs, bs).reshape(-1), neg_loss.transpose(-1, -
                                     2).expand(bs, bs).reshape(-1), target)  # margin loss

            kld_loss = (kl_divergence(h_experts['x_gnn_dist'][i], normal_dist).sum(-1).mean(dim=0) + kl_divergence(t_experts['x_gnn_dist']
                        [i], normal_dist).sum(-1).mean(dim=0) + kl_divergence(t_neg_experts['x_gnn_dist'][i], normal_dist).sum(-1).mean(dim=0))/3
        ...
        total_loss = {'kg_loss': kg_loss, 'kld_loss': loss_assist_kld, 'kg_cur_loss': loss_cur_kg,
                      'kg_assist_loss': loss_assist_kg, 'info_contrastive': info_contrastive,  'vq_loss': vq_loss}
        return total_loss
...
class TransE(nn.Module):
    ...
    def forward(self, h, r, t, t_neg):
        ...
        pos_loss = self.define_score([t, projected_t])  # [batch_size, 1]
        neg_loss = self.define_score([t, t_neg])  # [batch_size, 1]
